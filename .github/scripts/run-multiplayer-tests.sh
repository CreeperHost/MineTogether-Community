...
wait_for_log() {
  ...
  for ((second = 0; second < timeout; second++)); do
    ...
    if grep -Eq 'Minecraft exited with code: -?[1-9][0-9]*' "$log" 2>/dev/null; then
      fail "Process group $pgid exited unsuccessfully before '$pattern' appeared in $log"
    fi
...
start_client() {
  ...
  if [[ "$minecraft" == 1.12.* && "$port" -gt 0 ]]; then
    # QuickExitCli concatenates process argv before parsing the HMC command, so
    # literal quotes must survive into the command string for this multi-token option.
...
if [[ ! -d "$shared_mc/versions/$launch_regex" ]]; then
...
if run_scenario 7; then
  echo "Scenario 7/7: two MineTogether clients relay emotes over CTCP on a vanilla server"
  reset_results
...
assert_clean_logs
echo "All $loader singleplayer, offline connect, local chat, multiplayer compatibility, emote relay, CTCP fallback, and cleanup scenarios passed."
