...
assert_clean_logs() {
  ...
  # Headless NeoForge probes BSD kqueue availability and resolves its shutdown watchdog after module close.
  local known_nonfatal='dev[/\.]ftb[/\.]mods[/\.]ftbquests[/\.]client[/\.]FTBQuestsNetClient|io\.netty\.channel\.kqueue\.Native|Only supported on OSX/BSD.*(MT Connect Friend Search Executor|MT Connect Friend Share Executor|MT Server Connector #[0-9]+|Server thread)|Client shutdown watchdog.*net/minecraft/server/dedicated/ServerWatchdog|ClassNotFoundException: net\.minecraft\.server\.dedicated\.ServerWatchdog'
  local logs=()
