...
public record TailAnimation(
    ...
    public TailPose pose(AbstractClientPlayer player, float partialTicks, float ageInTicks) {
        ...
        if (player.isPassenger()) {
            ...
            float verticalLag = Mth.clamp((float) d1 * verticalLagScale, verticalLagMin, verticalLagMax);
            float backwardLag = Mth.clamp((float) (d0 * sin + d2 * back) * backwardLagScale, backwardLagMin, backwardLagMax);
            float sidewaysLag = Mth.clamp((float) (d0 * back - d2 * sin) * sidewaysLagScale, sidewaysLagMin, sidewaysLagMax);
