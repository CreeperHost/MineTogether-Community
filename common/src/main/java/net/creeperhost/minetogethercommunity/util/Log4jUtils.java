...
public class Log4jUtils {
    ...
    public static void attachMTLogs(Path logsFolder) {
        ...
        logsPath += "minetogethercommunity/";
        ...
        try {
            ...
            configuration.getRootLogger().addAppender(latest, Level.INFO, new LoggerNameStartsWithFilter("net.creeperhost.minetogethercommunity", Result.ACCEPT, Result.DENY));
            configuration.getRootLogger().addAppender(debug, Level.ALL, new LoggerNameStartsWithFilter("net.creeperhost.minetogethercommunity", Result.ACCEPT, Result.DENY));
            ctx.updateLoggers();
        } catch (Throwable ex) {
