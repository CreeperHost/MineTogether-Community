...
public class TailModel {
    ...
    public void render(TailPose pose, TailElementPose elementPose) {
        ...
        if (animatedChain) {
            renderAnimatedChain(buffer, pose, elementPose);
        } else {
            for (TailElement element : elements) {
                emitElement(buffer, element, null, null, elementPose);
            }
        }
        tessellator.draw();
    }
    ...
    private void emitElement(BufferBuilder buffer, TailElement element, float[] dynamicOrigin, float[] dynamicAngles, TailElementPose elementPose) {
        float x0 = element.from()[0], y0 = element.from()[1], z0 = element.from()[2];
        float x1 = element.to()[0], y1 = element.to()[1], z1 = element.to()[2];
        if (element.south() != null) emitFace(buffer, element.south(), verts(element, dynamicOrigin, dynamicAngles, elementPose, new float[][] {{x0, y0, z1}, {x1, y0, z1}, {x1, y1, z1}, {x0, y1, z1}}), normal(element, dynamicAngles, elementPose, 0, 0, 1));
        if (element.north() != null) emitFace(buffer, element.north(), verts(element, dynamicOrigin, dynamicAngles, elementPose, new float[][] {{x1, y0, z0}, {x0, y0, z0}, {x0, y1, z0}, {x1, y1, z0}}), normal(element, dynamicAngles, elementPose, 0, 0, -1));
        if (element.east() != null) emitFace(buffer, element.east(), verts(element, dynamicOrigin, dynamicAngles, elementPose, new float[][] {{x1, y0, z1}, {x1, y0, z0}, {x1, y1, z0}, {x1, y1, z1}}), normal(element, dynamicAngles, elementPose, 1, 0, 0));
        if (element.west() != null) emitFace(buffer, element.west(), verts(element, dynamicOrigin, dynamicAngles, elementPose, new float[][] {{x0, y0, z0}, {x0, y0, z1}, {x0, y1, z1}, {x0, y1, z0}}), normal(element, dynamicAngles, elementPose, -1, 0, 0));
        if (element.up() != null) emitFace(buffer, element.up(), verts(element, dynamicOrigin, dynamicAngles, elementPose, new float[][] {{x0, y1, z1}, {x1, y1, z1}, {x1, y1, z0}, {x0, y1, z0}}), normal(element, dynamicAngles, elementPose, 0, 1, 0));
        if (element.down() != null) emitFace(buffer, element.down(), verts(element, dynamicOrigin, dynamicAngles, elementPose, new float[][] {{x0, y0, z0}, {x1, y0, z0}, {x1, y0, z1}, {x0, y0, z1}}), normal(element, dynamicAngles, elementPose, 0, -1, 0));
    }

    private void renderAnimatedChain(BufferBuilder buffer, TailPose pose, TailElementPose elementPose) {
        ...
        for (TailElement element : elements) {
            ...
            emitElement(buffer, element, dynamicOrigin, dynamicAngles, TailElementPose.none());
